...
export default function HabitList() {
  ...
  // Load habits (incl. archived) into the store on mount so the list,
  // archive/restore, and delete actions have data to work with.
  useEffect(() => {
    loadHabits();
  }, [loadHabits]);
  ...
  return (
    <section className="flex flex-col rounded-card border border-border-subtle">
      {/* Collapsible header — collapsed by default so it takes minimal space */}
      <div className="flex items-center justify-between px-4 py-2.5">
        <button
          onClick={() => setExpanded((e) => !e)}
          className="flex items-center gap-2 text-h2 font-semibold text-text-primary hover:text-accent-primary transition-colors"
